...
def test_conv_forward():
    ...
    print(array_close(x.grad, xgrad, tol=1e-4, rtol=1e-4))
    print(array_close(kernel.grad, kgrad, tol=1e-5, rtol=1e-5))

    print('------------------------------------------------------')
    print('Test settings: stride=1, padding=2, dilation=1')
    print('Forward ....')
    x.release()
    kernel.release()
    pred = conv_fn(x, kernel, stride=1, padding=2)
    gt = np.array(
        [[[[1.1359, 3.5910, 5.7813, 5.7813, 5.7813, 4.6453, 2.1903],
           [3.2945, 8.3785, 11.7325, 11.7325, 11.7325, 8.4380, 3.3540],
           [5.2425, 12.0774, 17.7747, 17.7747, 17.7747, 12.5322, 5.6973],
           [5.2425, 12.0774, 17.7747, 17.7747, 17.7747, 12.5322, 5.6973],
           [5.2425, 12.0774, 17.7747, 17.7747, 17.7747, 12.5322, 5.6973],
           [4.1065, 8.4864, 11.9934, 11.9934, 11.9934, 7.8869, 3.5070],
           [1.9480, 3.6989, 6.0422, 6.0422, 6.0422, 4.0942, 2.3433]],
          [[1.6275, 3.7125, 6.1193, 6.1193, 6.1193, 4.4919, 2.4068],
           [4.3373, 8.6690, 11.9184, 11.9184, 11.9184, 7.5811, 3.2494],
           [5.7865, 12.3080, 16.9702, 16.9702, 16.9702, 11.1837, 4.6622],
           [5.7865, 12.3080, 16.9702, 16.9702, 16.9702, 11.1837, 4.6622],
           [5.7865, 12.3080, 16.9702, 16.9702, 16.9702, 11.1837, 4.6622],
           [4.1590, 8.5955, 10.8509, 10.8509, 10.8509, 6.6918, 2.2554],
           [1.4492, 3.6390, 5.0518, 5.0518, 5.0518, 3.6026, 1.4129]],
          [[1.7735, 3.3216, 5.4516, 5.4516, 5.4516, 3.6781, 2.1300],
           [3.4546, 6.1002, 10.8747, 10.8747, 10.8747, 7.4201, 4.7745],
           [4.7706, 8.3700, 14.7733, 14.7733, 14.7733, 10.0027, 6.4033],
           [4.7706, 8.3700, 14.7733, 14.7733, 14.7733, 10.0027, 6.4033],
           [4.7706, 8.3700, 14.7733, 14.7733, 14.7733, 10.0027, 6.4033],
           [2.9971, 5.0483, 9.3217, 9.3217, 9.3217, 6.3245, 4.2733],
           [1.3160, 2.2697, 3.8986, 3.8986, 3.8986, 2.5826, 1.6289]]],
         [[[1.1359, 3.5910, 5.7813, 5.7813, 5.7813, 4.6453, 2.1903],
           [3.2945, 8.3785, 11.7325, 11.7325, 11.7325, 8.4380, 3.3540],
           [5.2425, 12.0774, 17.7747, 17.7747, 17.7747, 12.5322, 5.6973],
           [5.2425, 12.0774, 17.7747, 17.7747, 17.7747, 12.5322, 5.6973],
    ...
    print(array_close(pred.values, gt, tol=1e-4, rtol=1e-4))

    print('Backward ...')
    loss = sum_fn(pred)
    loss.backward()
    xgrad = np.array(
        [[[[12.7189, 12.7189, 12.7189, 12.7189, 12.7189],
           [12.7189, 12.7189, 12.7189, 12.7189, 12.7189],
           [12.7189, 12.7189, 12.7189, 12.7189, 12.7189],
           [12.7189, 12.7189, 12.7189, 12.7189, 12.7189],
           [12.7189, 12.7189, 12.7189, 12.7189, 12.7189]],
          [[15.6485, 15.6485, 15.6485, 15.6485, 15.6485],
           [15.6485, 15.6485, 15.6485, 15.6485, 15.6485],
           [15.6485, 15.6485, 15.6485, 15.6485, 15.6485],
           [15.6485, 15.6485, 15.6485, 15.6485, 15.6485],
           [15.6485, 15.6485, 15.6485, 15.6485, 15.6485]],
          [[8.8229, 8.8229, 8.8229, 8.8229, 8.8229],
           [8.8229, 8.8229, 8.8229, 8.8229, 8.8229],
           [8.8229, 8.8229, 8.8229, 8.8229, 8.8229],
           [8.8229, 8.8229, 8.8229, 8.8229, 8.8229],
           [8.8229, 8.8229, 8.8229, 8.8229, 8.8229]],
          [[12.3279, 12.3279, 12.3279, 12.3279, 12.3279],
           [12.3279, 12.3279, 12.3279, 12.3279, 12.3279],
           [12.3279, 12.3279, 12.3279, 12.3279, 12.3279],
           [12.3279, 12.3279, 12.3279, 12.3279, 12.3279],
           [12.3279, 12.3279, 12.3279, 12.3279, 12.3279]]],
    ...
    print('------------------------------------------------------')
    print('Test settings: stride=1, padding=1, dilation=2')
    print('Forward ....')
    x.release()
    kernel.release()
    pred = conv_fn(x, kernel, stride=1, padding=1, dilation=2)
    gt = np.array(
        [[[[8.3785, 11.7325, 8.4380],
           [12.0774, 17.7747, 12.5322],
           [8.4864, 11.9934, 7.8869]],
    ...
    loss.backward()
    xgrad = np.array(
        [[[[1.3082, 3.3494, 2.0413, 3.1679, 1.1266],
           [2.0095, 5.3643, 3.3548, 6.2056, 2.8508],
           [0.7013, 2.0149, 1.3136, 3.0378, 1.7242],
           [2.4803, 5.2986, 2.8184, 5.7627, 2.9443],
           [1.7789, 3.2838, 1.5048, 2.7249, 1.2201]],
          [[1.7554, 3.3374, 1.5820, 2.3006, 0.7186],
           [3.7197, 7.2657, 3.5460, 6.4185, 2.8725],
           [1.9643, 3.9283, 1.9640, 4.1179, 2.1539],
           [3.9344, 7.7910, 3.8566, 7.6582, 3.8015],
           [1.9700, 3.8627, 1.8927, 3.5403, 1.6476]],
          [[1.0004, 1.4394, 0.4390, 1.9334, 1.4944],
           [2.0768, 3.5233, 1.4465, 4.1778, 2.7312],
           [1.0764, 2.0839, 1.0076, 2.2443, 1.2368],
           [2.1491, 4.3847, 2.2356, 3.7400, 1.5043],
           [1.0727, 2.3008, 1.2281, 1.4956, 0.2675]],
          [[1.3211, 2.1533, 0.8322, 2.2057, 1.3735],
           [2.2298, 4.7499, 2.5201, 5.3283, 2.8082],
           [0.9087, 2.5966, 1.6879, 3.1226, 1.4347],
           [2.8142, 5.9648, 3.1506, 5.9869, 2.8363],
           [1.9054, 3.3681, 1.4627, 2.8643, 1.4016]]],
         [[[1.3082, 3.3494, 2.0413, 3.1679, 1.1266],
           [2.0095, 5.3643, 3.3548, 6.2056, 2.8508],
           [0.7013, 2.0149, 1.3136, 3.0378, 1.7242],
           [2.4803, 5.2986, 2.8184, 5.7627, 2.9443],
